...
public class PriorityScheduler extends Scheduler {
	...
	public void setPriority(KThread thread, int priority) {
		...
		Lib.my_assert(priority >= priorityMinimum
	...
	protected class PriorityQueue extends ThreadQueue {
		...
		protected ThreadState peekNextThread() {
			...
			if (waiting.containsKey(pr)) {
				List<ThreadState> list = waiting.get(pr);
		...
		protected void putToWait(ThreadState ts) {
			...
			if (!waiting.containsKey(priority)) {
				waiting.put(priority, new LinkedList<ThreadState>());
			}
			List<ThreadState> queue = waiting.get(priority);
			
			//check if we are not already in the queue
			Lib.my_assert(queue.contains(ts));
